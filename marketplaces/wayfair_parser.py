...
class WayfairParser:
    ...
    def parse_order(self) -> list[dict[str, None | str | int]]:
        ...
        order_items = []
        items = (self.soup.find("tbody", attrs={"data-hb-id": "TableBody"}).
                 find_all("tr", attrs={"data-hb-id": "TableRow"}))
    ...
    def __get_order_id(self) -> str:
        ...
        try:
            order_id = self.soup.find("h1", class_="b62nt514o mb5j687 mb5j68d mb5j68v",
                                      attrs={"data-hb-id": "Heading"}).text.strip()
    ...
    def __get_store_title(self):
        ...
        try:
            store_title = self.soup.find_all("strong", {"data-tag-default": "order-details_orderDetails_strong"})[
                -1].text.strip()
    ...
    def __get_items_total(self) -> float | str:
        ...
        try:
            items_total = self.soup.find_all("strong", attrs={"data-tag-default": "order-details_orderDetails_Text"})[
                4].text.strip()
    ...
    def __get_tracking_number(self) -> str:
        ...
        try:
            tracking_number = self.soup.find_all("p", attrs={"data-tag-default": "order-details_orderDetails_Text"})[
                -1].text.strip()
            if tracking_number == "Tracking Number(s)":
                raise AttributeError(Fore.RED + "||| Не смог получить трек-номер |||" + Back.WHITE)
    ...
    def __ship_by_date(self) -> str:
        ...
        try:
            ship_by_date = self.soup.find_all("strong", attrs={"data-tag-default": "order-details_orderDetails_Text"})[
                1].text.strip()
    ...
    def __get_listing_links(self) -> list[str]:
        ...
        listing_urls = []
        try:
            listing_url_matches = re.finditer(r'(https://www\.wayfair\.com/[^?\s]+)', self.order)
    ...
    def __get_quantity(item) -> int | str:
        ...
        try:
            quantity_div = item.find_all("p", attrs={
                "data-tag-default": "order-details_useOrderItemsTableColumns_Text",
                "data-hb-id": "Text"
            })
            quantities = [
