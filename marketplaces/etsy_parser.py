...
class EtsyParser:
    ...
    def __get_sku(item: Any, listing_title: str) -> str:
        ...
        try:
            ...
            print(
                Fore.YELLOW
                + "SKU не указан на листинге, пытаюсь получить его из названия товара"
                + Back.WHITE
            )
        ...
        try:
            ...
            for span in address_div.find_all("span"):
                class_name = span.get("class", [None])[
                    0
                ]  # Получаем класс первого элемента, если он есть
                if class_name == "name":
            ...
            print(
                Fore.GREEN
                + f"- Адрес клиента:\n{Fore.MAGENTA}{full_address}{Back.WHITE}"
                + Back.WHITE
            )
            return full_address
    ...
    def __get_customization(item: Any) -> LiteralString | None:
        ...
        try:
            ...
            print(
                Fore.GREEN
                + f"- Кастомизация:\n{Fore.MAGENTA}{customization_items}{Back.WHITE}"
                + Back.WHITE
            )
    ...
    def __get_size(item: Any) -> str:
        ...
        try:
            ...
            if size_pattern and len(size_pattern) >= 2:
                ...
                print(
                    Fore.GREEN
                    + f"- Размер товара: {Fore.MAGENTA}{size} inches{Back.WHITE}"
                )
                return size
            else:
                print(
                    Fore.YELLOW + "||| Не удалось распознать размер из текста: ",
                    size_text,
                    " |||" + Back.WHITE,
                )
                return "!ERROR!"
                ...
                for el in postal_service_div:
                    ...
                    for p in paragraphs:
                        if "Shipped" in p.text or "Shipping" in p.text:
                            ...
                            print(
                                Fore.GREEN
                                + f"- Название почтовой службы (в случае, если доставка куплена не на Etsy): "
                                f"{Fore.MAGENTA}{postal_service}{Back.WHITE}"
                                + Back.WHITE
                            )
                            return postal_service
            except AttributeError:
    ...
    def __get_tracking_link(
        ...
        try:
            if postal_service == "USPS":
                ...
                tracking_link = (
                    self.soup.find("div", class_="col-xs-9 wt-wrap")
                    .find("a")
                    .get("href")
                )
                return tracking_link
